# THIRD-PARTY-LICENSE-REVIEW

Generated from: ../../fixture/node_modules
Main file: THIRD-PARTY-LICENSE.md

## @scope/b@2.0.0
- Main: THIRD-PARTY-LICENSE.md#pkg-scope-b-2-0-0
- Source: git+https://github.com/example/b.git
- License: Apache-2.0 (https://apache.org/licenses/LICENSE-2.0)
- Files:
  - COPYING
- Status: Present in node_modules
- Notes:

## dup-pkg@1.0.0 (@scope/b/node_modules/dup-pkg)
- Main: THIRD-PARTY-LICENSE.md#pkg-dup-pkg-1-0-0-scope-b-node-modules-dup-pkg
- Source: https://example.com/dup
- License: ISC
- Files:
  - LICENSE
- Status: Present in node_modules
- Notes:

## dup-pkg@1.0.0 (dup-pkg)
- Main: THIRD-PARTY-LICENSE.md#pkg-dup-pkg-1-0-0-dup-pkg
- Source: https://example.com/dup
- License: ISC
- Files:
  - LICENSE
- Status: Present in node_modules
- Notes:

## has-optional@1.0.0
- Main: THIRD-PARTY-LICENSE.md#pkg-has-optional-1-0-0
- Source: https://example.com/ho
- License: MIT
- Files:
  - LICENSE.txt
- Status: Present in node_modules
- Notes:

## linked-pkg@1.0.0
- Main: THIRD-PARTY-LICENSE.md#pkg-linked-pkg-1-0-0
- Source: https://example.com/linked
- License: MIT
- Files:
  - LICENSE
- Status: Present in node_modules
- Notes:

## multi-lic@3.1.4
- Main: THIRD-PARTY-LICENSE.md#pkg-multi-lic-3-1-4
- Source: https://example.com/multi
- License: MIT | BSD-2-Clause (https://example.com/bsd)
- Files:
  - LICENSE
- Status: Present in node_modules
- Notes:

## opt-pkg@1.0.0
- Main: THIRD-PARTY-LICENSE.md#pkg-opt-pkg-1-0-0
- Source: (missing)
- License: (missing)
- Files:
  - (none)
- Status: Present in node_modules / Missing Source / Missing package.json license / Missing LICENSE/NOTICE/COPYRIGHT/THIRD-PARTY-NOTICES/THIRD-PARTY-LICENSES/ThirdPartyNoticeText/ThirdPartyText/COPYING files
- Notes:

## pkg-a@1.0.0
- Main: THIRD-PARTY-LICENSE.md#pkg-pkg-a-1-0-0
- Source: github:example/pkg-a
- License: MIT
- Files:
  - LICENSE
  - NOTICE.md
- Status: Present in node_modules
- Notes:

---

## Missing summary

### Missing Source

- opt-pkg@1.0.0

### Missing package.json license field

- opt-pkg@1.0.0

### Missing LICENSE/NOTICE/COPYRIGHT/THIRD-PARTY-NOTICES/THIRD-PARTY-LICENSES/ThirdPartyNoticeText/ThirdPartyText/COPYING files

- opt-pkg@1.0.0

